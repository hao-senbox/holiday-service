...
type TermGateway interface {
	...
	GetCurrentTermByOrgID(ctx context.Context, orgID string) (*response.TermResponse, error)
}
...
func (g *termGatewayImpl) GetCurrentTermByOrgID(ctx context.Context, orgID string) (*response.TermResponse, error) {
	token, ok := ctx.Value(constants.TokenKey).(string)
	if !ok {
		return nil, fmt.Errorf("token not found in context")
	}
	...
	resp, err := client.Call("GET", "/api/v1/gateway/terms/current/"+orgID, nil)
	if err != nil {
		return nil, fmt.Errorf("call API term fail: %w", err)
	}
